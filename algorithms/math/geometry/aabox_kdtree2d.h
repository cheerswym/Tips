...
#ifndef ONBOARD_MATH_GEOMETRY_AABOX_KDTREE2D_H_
#define ONBOARD_MATH_GEOMETRY_AABOX_KDTREE2D_H_
...
class AABoxKDTree2dNode {
 ...
 private:
  void InitObjects(std::vector<ObjectPtr> objects) {
    num_objects_ = static_cast<int>(objects.size());
    objects_sorted_by_min_ = objects;
    objects_sorted_by_max_ = std::move(objects);
    ...
    if (partition_ == PARTITION_X) {
      std::sort(objects_sorted_by_min_.begin(), objects_sorted_by_min_.end(),
                [](ObjectPtr obj1, ObjectPtr obj2) {
                  return obj1->aabox().min_x() < obj2->aabox().min_x();
                });
      for (ObjectPtr object : objects_sorted_by_min_) {
        objects_sorted_by_min_bound_.push_back(object->aabox().min_x());
      }

      std::sort(objects_sorted_by_max_.begin(), objects_sorted_by_max_.end(),
                [](ObjectPtr obj1, ObjectPtr obj2) {
                  return obj1->aabox().max_x() > obj2->aabox().max_x();
                });
      for (ObjectPtr object : objects_sorted_by_max_) {
        objects_sorted_by_max_bound_.push_back(object->aabox().max_x());
      }
    } else {
      std::sort(objects_sorted_by_min_.begin(), objects_sorted_by_min_.end(),
                [](ObjectPtr obj1, ObjectPtr obj2) {
                  return obj1->aabox().min_y() < obj2->aabox().min_y();
                });
      for (ObjectPtr object : objects_sorted_by_min_) {
        objects_sorted_by_min_bound_.push_back(object->aabox().min_y());
      }

      std::sort(objects_sorted_by_max_.begin(), objects_sorted_by_max_.end(),
                [](ObjectPtr obj1, ObjectPtr obj2) {
                  return obj1->aabox().max_y() > obj2->aabox().max_y();
                });
      for (ObjectPtr object : objects_sorted_by_max_) {
        objects_sorted_by_max_bound_.push_back(object->aabox().max_y());
      }
    }
  }
  ...
  void ComputeBoundary(const std::vector<ObjectPtr> &objects) {
    min_x_ = objects[0]->aabox().min_x();
    min_y_ = objects[0]->aabox().min_y();
    max_x_ = objects[0]->aabox().max_x();
    max_y_ = objects[0]->aabox().max_y();
    for (int i = 1; i < objects.size(); ++i) {
      min_x_ = std::fmin(min_x_, objects[i]->aabox().min_x());
      max_x_ = std::fmax(max_x_, objects[i]->aabox().max_x());
      min_y_ = std::fmin(min_y_, objects[i]->aabox().min_y());
      max_y_ = std::fmax(max_y_, objects[i]->aabox().max_y());
    }
 ...
 private:
  ...
  std::unique_ptr<AABoxKDTree2dNode<ObjectType>> left_subnode_;
  std::unique_ptr<AABoxKDTree2dNode<ObjectType>> right_subnode_;
};
...
#endif  // ONBOARD_MATH_GEOMETRY_AABOX_KDTREE2D_H_
