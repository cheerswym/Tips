...
TEST(AABox2dTest, GetAllCorners) {
  ...
  EXPECT_THAT(box1, AABox2dEq(Vec2d(0.0, 0.0), /*length=*/4.0, /*width=*/2.0));
  ...
  EXPECT_THAT(box2, AABox2dEq(Vec2d(5.000000, 2.000000),
                              /*length=*/4, /*width=*/2));
}
...
TEST(AABox2dTest, ComputeOverlapArea) {
  AABox2d box1({0, 0}, 4, 2);
  AABox2d box2({3, 1}, {7, 3});
  AABox2d box3({0, 0}, 10, 10);
  EXPECT_NEAR(box1.ComputeOverlapArea(box2), 0., 1e-5);
  EXPECT_NEAR(box1.ComputeOverlapArea(box3), 8.0, 1e-5);
  EXPECT_NEAR(box2.ComputeOverlapArea(box3), 4.0, 1e-5);
}
...
TEST(AABox2dTest, SquaredDistanceTo) {
  AABox2d box({1, 2}, 4, 2);
  EXPECT_NEAR(box.SquaredDistanceTo({3, 0}), 1.0, 1e-5);
  EXPECT_NEAR(box.SquaredDistanceTo({4, 0}), 2.0, 1e-5);
  EXPECT_NEAR(box.SquaredDistanceTo({5, 2}), 4.0, 1e-5);
  EXPECT_NEAR(box.SquaredDistanceTo({-3, 2}), 4.0, 1e-5);
  EXPECT_NEAR(box.SquaredDistanceTo({0, 2}), 0.0, 1e-5);
  EXPECT_NEAR(box.SquaredDistanceTo({0, -2}), 9.0, 1e-5);
  EXPECT_NEAR(box.SquaredDistanceTo({0, -1}), 4.0, 1e-5);
  EXPECT_NEAR(box.SquaredDistanceTo({-3, -2}), 13.0, 1e-5);
}
