...
void LineFitter::FitData(FITTER fitter, bool normalize, bool compute_mse) {
  ...
  // debug
  if (debug_) {
    VLOG(0) << "******** line fitter stats ********";
    VLOG(0) << " line fitter data mean: " << sample_mean.transpose();
    VLOG(0) << " line fitter data cov: " << sample_cov;
    VLOG(0) << "******** line fitter stats ********";
  }
  ...
  if (fitter == FITTER::DEMING) {
    ...
    const Vec2d line_normal = svd.matrixU().block<2, 1>(0, 1);
  ...
  if (normalize) {
    ...
    if (debug_) {
      VLOG(0) << "  normalized param = " << param_.transpose();
    }
  }
