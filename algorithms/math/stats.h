#ifndef ONBOARD_MATH_STATS_H_
#define ONBOARD_MATH_STATS_H_
...
Eigen::Matrix<T, 2, 2> Weighted2DSampleMeanAndCovarianceMatrix(
  ...
  QCHECK(sample_mean != nullptr);
  QCHECK(!has_weight || samples.size() == sample_weights.size());
  *sample_mean =
