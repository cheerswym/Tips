...
ProjectionResult ProjectionToLanePath(Vec2d xy, double start_s, double end_s,
...
LanePathInfo::LanePathInfo(mapping::LanePath lane_path, double len_along_route,
    ...
    : length_along_route_(len_along_route),
      ...
      anchor_points_(mapping::SampleLanePathPoints(smm, lane_path)) {
...
FrenetCoordinate LanePathInfo::ProjectionSL(Vec2d xy) const {
  const auto res =
...
FrenetCoordinate LanePathInfo::ProjectionSLInRange(Vec2d xy, double start_s,
                                                   double end_s) const {
