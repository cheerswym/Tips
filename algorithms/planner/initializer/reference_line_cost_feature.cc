...
double Cubic(double v) { return v * v * v; }
}  // namespace
...
void RefLineProgressFeatureCost::ComputeCost(const GeometryEdgeInfo& edge_info,
  ...
  if (edge_info.terminating) {
    const auto& final_state = edge_info.states.back();
...
void RefLinePathBoundaryFeatureCost::ComputeCost(
  ...
  for (const auto& state : edge_info.states) {
    ...
    cost[1] += 1.2 * Sqr(l_target_right_ratio) + Sqr(l_target_left_ratio);
    // Boundary.
    cost[2] += 1.2 * Sqr(l_right_ratio) + Sqr(l_left_ratio);
  }

  cost[1] = cost[1] / edge_info.states.size();
  cost[2] = cost[2] / edge_info.states.size();
}
...
void RefLineCurvatureFeatureCost::ComputeCost(const GeometryEdgeInfo& edge_info,
  ...
  cost[0] = cost_sum / edge_info.states.size();
}
