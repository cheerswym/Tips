...
std::pair<double, double> XY(
    ...
    const QuinticSpiralBoundaryValueProblemParams& kParams) {
  const double resolution = 1.0 / kParams.num_steps;
  const double h = sg * resolution;
  double fx = 0.0;
  double fy = 0.0;
  Vec3d fx_grad;
  Vec3d fy_grad;
  for (int i = 0; i <= kParams.num_steps; i++) {
