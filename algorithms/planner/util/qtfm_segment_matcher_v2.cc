...
std::optional<Vec2d> LineIntersection(const Segment2d& l0,
                                      const Segment2d& l1) {
  Vec2d res = FindIntersectionBetweenLinesWithTangents(
      l0.start(), l0.unit_direction(), l1.start(), l1.unit_direction());
...
QtfmSegmentMatcherV2::QtfmSegmentMatcherV2(
    Config config, const std::vector<Segment2d>& raw_segments,
  ...
  MakeSegmentsDivisible(raw_segments, raw_start_segment_ids, &segments_,
...
void QtfmSegmentMatcherV2::MakeSegmentsDivisible(
  ...
  divisible_start_segment_ids->reserve(raw_start_segment_ids.size());
