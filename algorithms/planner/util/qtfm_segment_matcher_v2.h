...
#include "onboard/container/small_vector.h"
...
class QtfmSegmentMatcherV2 {
 public:
  ...
  struct Config {
    ...
    // Region of interest.
    // Querys outside the domain is not guaranteed to be correct.
    double x_min = 0.0;
    double x_max = 0.0;
    double y_min = 0.0;
    double y_max = 0.0;
    ...
    // The matcher's structure is a sparse gird map of
    ...
    // and the max number of sinlge range candidates <=
    // cutoff_max_num_candidate. The field will be deemed as simple enougth,
 ...
 private:
  ...
  // Neighbor ranges canbe stiched to make larger range, when in box.
