...
namespace qcraft {
namespace eval {
TEST(TimestampGrouperUnitTest, empty_qevent) {
  ...
  TimestampGrouper timestamp_grouper({grouper});
  const auto &result = timestamp_grouper.Group(qevents);
  ASSERT_EQ(result.size(), 0);
}

TEST(TimestampGrouperUnitTest, normal_ok) {
  std::vector<std::vector<QEventProto>> qevents(2, std::vector<QEventProto>());
...
TEST(TimestampGrouperUnitTest, zero_interval) {
  std::vector<std::vector<QEventProto>> qevents(2, std::vector<QEventProto>());
  QEventProto event1, event2, event3;
  *event1.mutable_uid() = "1";
  event1.set_timestamp(123456789);
  ...
  *event3.mutable_uid() = "3";
  ...
  QEventGrouper grouper;
  grouper.set_name("timestamp_grouper");
  grouper.mutable_timestamp_grouper();
  TimestampGrouper timestamp_grouper({grouper});
  const auto &result = timestamp_grouper.Group(qevents);
  ASSERT_EQ(result.size(), 3);
  ASSERT_EQ(result[0].selections(0).events(0).uid(), "3");
  ASSERT_EQ(result[0].selections(1).events_size(), 0);
  ASSERT_EQ(result[1].selections(1).events(0).uid(), "2");
  ASSERT_EQ(result[1].selections(0).events_size(), 0);
  ASSERT_EQ(result[2].selections(0).events(0).uid(), "1");
  ASSERT_EQ(result[2].selections(1).events_size(), 0);
}
}  // namespace eval
}  // namespace qcraft
