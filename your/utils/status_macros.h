...
#ifndef ONBOARD_UTILS_STATUS_MACROS_H_
#define ONBOARD_UTILS_STATUS_MACROS_H_
...
#define RETURN_IF_ERROR(expr)                                             \
  ...
  if (qcraft::status_macro_internal::StatusAdaptorForMacros               \
...
#define STATUS_MACROS_IMPL_ASSIGN_OR_RETURN_(statusor, lhs, rexpr,      \
  ...
  if (ABSL_PREDICT_FALSE(!statusor.ok())) {                             \
    StatusBuilder _(std::move(statusor).status(), __FILE__, __LINE__);  \
...
#define STATUS_MACROS_IMPL_ASSIGN_OR_DIE_(statusor, lhs, rexpr,         \
  ...
  if (ABSL_PREDICT_FALSE(!statusor.ok())) {                             \
    StatusBuilder _(std::move(statusor).status(), __FILE__, __LINE__);  \
    (void)_; /* error_expression is allowed to not use this variable */ \
    QCHECK(false) << (_ << error_expression).JoinMessageToStatus();     \
  }                                                                     \
  lhs = std::move(statusor).value()
...
#endif  // ONBOARD_UTILS_STATUS_MACROS_H_
