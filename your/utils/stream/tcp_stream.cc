...
bool TcpStream::InitSocket() {
  ...
  if (timeout_usec_ != 0) {
    ...
    if (flags == -1) {
      ::close(sockfd_);
      QLOG(ERROR) << "fcntl get flag failed, error: " << strerror(errno);
      return false;
    }

    if (fcntl(sockfd_, F_SETFL, flags & ~O_NONBLOCK) == -1) {
      ::close(sockfd_);
      QLOG(ERROR) << "fcntl set block failed, error: " << strerror(errno);
      return false;
    }
    ...
    if (setsockopt(sockfd_, SOL_SOCKET, SO_RCVTIMEO, &block_to,
                   sizeof(block_to)) < 0) {
      ::close(sockfd_);
      QLOG(ERROR) << "setsockopt set rcv timeout failed, error: "
                  << strerror(errno);
      return false;
    }
    ...
    if (setsockopt(sockfd_, SOL_SOCKET, SO_SNDTIMEO, &block_to,
                   sizeof(block_to)) < 0) {
      ::close(sockfd_);
      QLOG(ERROR) << "setsockopt set snd timeout failed, error: "
                  << strerror(errno);
      return false;
    }
  } else {
    ...
    if (fcntl(sockfd_, F_SETFL, flags | O_NONBLOCK) == -1) {
      ::close(sockfd_);
      QLOG(ERROR) << "fcntl set non block failed, error: " << strerror(errno);
      return false;
    }
  }
  ...
  if (ret == -1) {
    ::close(sockfd_);
    QLOG(ERROR) << "setsockopt disable Nagle failed, errno: " << errno << ", "
                << strerror(errno);
    return false;
  }
...
bool TcpStream::Connect() {
  ...
  if (fd_flags < 0 || fcntl(sockfd_, F_SETFL, fd_flags | O_NONBLOCK) < 0) {
    QLOG(ERROR) << "Failed to set noblock, error: " << strerror(errno);
    return false;
  }
  ...
  while ((ret = ::connect(sockfd_, reinterpret_cast<sockaddr*>(&peer_addr),
                          sizeof(peer_addr))) < 0) {
    if (errno == EINTR) {
      QLOG(INFO) << "Tcp connect return EINTR, continue.";
      continue;
    } else {
      if ((errno != EISCONN) && (errno != EINPROGRESS) && (errno != EALREADY)) {
        status_ = Stream::Status::ERROR;
        errno_ = errno;
        QLOG(ERROR) << "Connect failed, error: " << strerror(errno);
        return false;
      }
      ...
      if (ret < 0) {
        status_ = Stream::Status::ERROR;
        errno_ = errno;
        QLOG(ERROR) << "Wait connect failed, error: " << strerror(errno);
        return false;
      } else if (ret == 0) {
        QLOG(INFO) << "Tcp connect timeout.";
        return false;
        ...
        if (getsockopt(sockfd_, SOL_SOCKET, SO_ERROR, &error, &len) < 0) {
          status_ = Stream::Status::ERROR;
          errno_ = errno;
          QLOG(ERROR) << "Getsockopt failed, error: " << strerror(errno);
          return false;
        }
        if (error != 0) {
          status_ = Stream::Status::ERROR;
          errno_ = errno;
          QLOG(ERROR) << "Socket error: " << strerror(errno);
          return false;
        }
  ...
  if (!InitSocket()) {
    ...
    status_ = Stream::Status::ERROR;
    errno_ = errno;
    QLOG(ERROR) << "Failed to init socket.";
    return false;
  }
  QLOG(INFO) << "Tcp connect success.";
...
size_t TcpStream::read(uint8_t* buffer, size_t max_length) {
  ...
  while ((ret = ::recv(sockfd_, buffer, max_length, 0)) < 0) {
    if (errno == EINTR) {
      ...
      if (errno != EAGAIN) {
        status_ = Stream::Status::ERROR;
        errno_ = errno;
        QLOG(ERROR) << "Read errno " << errno << ", error " << strerror(errno);
      }
    }
  ...
  if (ret == 0) {
    status_ = Stream::Status::ERROR;
    errno_ = errno;
    QLOG(ERROR) << "Remote closed.";
    if (Reconnect()) {
      QLOG(INFO) << "Reconnect tcp success.";
    }
  }
...
bool TcpStream::Readable(uint32_t timeout_us) {
  ...
  if (r < 0) {
    status_ = Stream::Status::ERROR;
    errno_ = errno;
    QLOG(ERROR) << "Failed to wait tcp data: " << errno << ", "
                << strerror(errno);
    return false;
