...
speed_t SerialStream::get_serial_baudrate(uint32_t rate) {
  switch (rate) {
    case 9600:
      return B9600;
    case 19200:
...
void SerialStream::open(void) {
  ...
  if (fd == -1) {
    switch (errno) {
      ...
      case EMFILE:
      default:
        QLOG(ERROR) << "Open device " << device_name_
...
void SerialStream::check_remove() {
  ...
  if (nsent < 0) {
    QLOG(ERROR) << "Serial stream detect write failed, error: "
                << strerror(errno);
    switch (errno) {
      case EBADF:
      case EIO:
        status_ = Stream::Status::DISCONNECTED;
        QLOG(ERROR) << "Device " << device_name_ << " removed.";
        Disconnect();
        break;
    }
  }
...
size_t SerialStream::read(uint8_t* buffer, size_t max_length) {
  if (!is_open_) {
    ...
    QLOG(INFO) << "Connect " << device_name_ << " success.";
  }
  ...
  while (max_length > 0) {
    ...
    if (bytes_current_read < 0) {
      switch (errno) {
        ...
        case EBADF:
        case EIO:
          QLOG(ERROR) << "Serial stream read data failed, error: "
                      << strerror(errno);
          Disconnect();
          if (Connect()) {
            QLOG(INFO) << "Reconnect " << device_name_ << " success.";
            bytes_current_read = 0;
...
size_t SerialStream::write(const uint8_t* data, size_t length) {
  ...
  while ((length > 0) && (delay_times < 5)) {
    ...
    if (nsent < 0) {
      QLOG(ERROR) << "Serial stream write data failed, error: "
                  << strerror(errno);
      switch (errno) {
        case EAGAIN:
        case EINVAL:
