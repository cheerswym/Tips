#ifndef ONBOARD_CONTROL_CONTROLLERS_TS_PKMPC_CONTROLLER_H_
#define ONBOARD_CONTROL_CONTROLLERS_TS_PKMPC_CONTROLLER_H_
...
class TsPkmpcController : public ControllerBase {
 public:
  TsPkmpcController();
  ...
  std::string_view Name() const override {
    return "Time-space Purely Kinematic MPC";
  }
 ...
 private:
  ...
  void FindSControlReference(const VehicleStateProto &vehicle_state,
                             const TrajectoryInterface &trajectory_interface,
                             SimpleMPCDebug *debug,
                             ControllerDebugProto *controller_debug_proto);

  void SControlUpdateInitialStateAndMatrix(
  ...
  static constexpr int kSControlStateNum = 3;
  static constexpr int kSControlInputNum = 1;

  int s_control_dead_steps_;
  double steering_delay_steps_;
...
#endif  // ONBOARD_CONTROL_CONTROLLERS_TS_PKMPC_CONTROLLER_H_
